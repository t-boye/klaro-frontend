...
const COUNTRY_REGIONS = {
  GH: ['Greater Accra', 'Ashanti', 'Western', 'Central', 'Eastern', 'Northern', 'Upper East', 'Upper West', 'Volta', 'Brong-Ahafo', 'Savannah', 'Bono East', 'Ahafo', 'Western North', 'Oti', 'North East'],
  NG: ['Lagos', 'Abuja (FCT)', 'Kano', 'Rivers', 'Oyo', 'Kaduna', 'Ogun', 'Delta', 'Anambra', 'Enugu', 'Imo', 'Cross River', 'Borno', 'Plateau', 'Kwara'],
  ZA: ['Gauteng', 'Western Cape', 'KwaZulu-Natal', 'Eastern Cape', 'Limpopo', 'Mpumalanga', 'North West', 'Free State', 'Northern Cape'],
  KE: ['Nairobi', 'Mombasa', 'Kisumu', 'Nakuru', 'Eldoret', 'Nyeri', 'Machakos', 'Meru', 'Kilifi', 'Kakamega'],
  RW: ['Kigali', 'Northern Province', 'Southern Province', 'Eastern Province', 'Western Province'],
  CI: ['Abidjan', 'Bouaké', 'Daloa', 'Korhogo', 'Yamoussoukro', 'San-Pédro', 'Divo', 'Man'],
  SN: ['Dakar', 'Thiès', 'Saint-Louis', 'Ziguinchor', 'Kaolack', 'Mbour', 'Touba', 'Diourbel'],
  EG: ['Cairo', 'Alexandria', 'Giza', 'Port Said', 'Suez', 'Luxor', 'Aswan', 'Asyut', 'Mansoura'],
  TZ: ['Dar es Salaam', 'Mwanza', 'Arusha', 'Dodoma', 'Mbeya', 'Morogoro', 'Tanga', 'Zanzibar'],
};

const COUNTRY_BAR_LABELS = {
  GH: 'GBA number (Ghana Bar Association)',
  NG: 'NBA number (Nigerian Bar Association)',
  ZA: 'Law Society number',
  KE: 'LSK number (Law Society of Kenya)',
  RW: 'RBA number (Rwanda Bar Association)',
  CI: 'CNB number (Conseil National du Barreau)',
  SN: "Ordre number (Ordre des Avocats du Sénégal)",
  EG: 'Egyptian Bar Association number',
  TZ: 'TLS number (Tanganyika Law Society)',
};

const COUNTRY_CURRENCIES = {
  GH: 'GH₵', NG: '₦', ZA: 'R', KE: 'KSh', RW: 'RWF', CI: 'XOF', SN: 'XOF', EG: 'EGP', TZ: 'TZS',
};
...
function LawyerCard({ lawyer }) {
  ...
  const barNum = lawyer.gba_number;
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden hover:shadow-md hover:border-brand-200 dark:hover:border-brand-700 transition-all">
      <div className="flex items-start gap-4 p-5">
        <div className="w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center text-white font-bold text-sm" style={{ background: color }}>
          ...
          <div className="flex items-start justify-between gap-2 flex-wrap">
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                {lawyer.years_experience > 0 && (
                  <span className="text-xs text-gray-400 dark:text-gray-500">{lawyer.years_experience} {t('lawyers.yrsExp')}</span>
                )}
                {barNum && <span className="text-xs text-gray-300 dark:text-gray-600">#{barNum}</span>}
              </div>
            </div>
            {lawyer.consultation_fee_ghs > 0 && (
              <div className="text-right flex-shrink-0 bg-gray-50 dark:bg-gray-700/50 rounded-xl px-3 py-1.5">
                <p className="text-xs text-gray-400 dark:text-gray-500 leading-tight">{t('lawyers.consultation')}</p>
                <p className="text-sm font-bold text-brand-600 dark:text-brand-400 leading-tight">
                  {COUNTRY_CURRENCIES[lawyer.country] || 'GH₵'}{Number(lawyer.consultation_fee_ghs).toFixed(0)}
                </p>
              </div>
            )}
          </div>
          {lawyer.specialties?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {lawyer.specialties.map(s => (
                <span key={s} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full px-2.5 py-0.5">{s}</span>
              ))}
            </div>
          )}
        </div>
      ...
      {lawyer.bio && (
        <div className="px-5 pb-4 border-t border-gray-50 dark:border-gray-700/50 pt-3">
          ...
          {lawyer.bio.length > 130 && (
            <button onClick={() => setExpanded(!expanded)} className="text-xs text-brand-600 dark:text-brand-400 mt-1 hover:underline font-medium">
              {expanded ? t('lawyers.readLess') : t('lawyers.readMore')}
            </button>
          )}
        </div>
      )}
...
const LANGUAGES_OPTIONS = [
  ...
  { code: 'fan', label: 'Fante'    }, { code: 'sw',  label: 'Swahili' },
  { code: 'fr',  label: 'French'   }, { code: 'ar',  label: 'Arabic'  },
];

const EMPTY_FORM = {
  full_name: '', email: '', phone: '', whatsapp: '', gba_number: '',
  country: '', region: '', specialties: [], years_experience: '',
  consultation_fee_ghs: '', languages: ['en'], bio: '', docs_url: '',
};

const STEPS = ['Personal info', 'Practice details', 'Verification'];

function ApplyModal({ onClose, userCountry }) {
  const [step,    setStep]    = useState(0);
  const [form,    setForm]    = useState({ ...EMPTY_FORM, country: userCountry || '' });
  ...
  const barLabel  = COUNTRY_BAR_LABELS[form.country] || 'Bar registration number';
  const currency  = COUNTRY_CURRENCIES[form.country] || '';

  function validateStep() {
    if (step === 0) {
      if (!form.country.trim())     return 'Select your country.';
      if (!form.full_name.trim())   return 'Full name is required.';
      if (!form.email.trim())       return 'Email is required.';
      if (!form.phone.trim())       return 'Phone number is required.';
      if (!form.gba_number.trim())  return 'Bar registration number is required.';
    }
    if (step === 1) {
            ...
            <div className="flex items-center gap-0">
              {STEPS.map((label, i) => (
                <React.Fragment key={i}>
                  ...
                  {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-2 transition-colors ${i < step ? 'bg-brand-600' : 'bg-gray-200 dark:bg-gray-700'}`} />}
                </React.Fragment>
              ))}
            </div>
          )}
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Country *</label>
                    <select
                      required
                      value={form.country}
                  ...
                  <div className="col-span-2">
                    ...
                    <input required value={form.full_name} onChange={e => set('full_name', e.target.value)} className="input text-sm w-full" placeholder="e.g. Amara Diallo" />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">{barLabel} *</label>
                    <input required value={form.gba_number} onChange={e => set('gba_number', e.target.value)} className="input text-sm w-full" placeholder="e.g. 2015/0412" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Phone number *</label>
                    <input required value={form.phone} onChange={e => set('phone', e.target.value)} className="input text-sm w-full" placeholder="+XXX XX XXX XXXX" />
                  </div>
                  <div>
                    ...
                    <input value={form.whatsapp} onChange={e => set('whatsapp', e.target.value)} className="input text-sm w-full" placeholder="+XXX XX XXX XXXX" />
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="p-6 space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">
                      Consultation fee{currency ? ` (${currency})` : ''}
                    </label>
                ...
                <div>
                  ...
                  <textarea required value={form.bio} onChange={e => set('bio', e.target.value)} rows={4} className="input text-sm w-full resize-none"
                    placeholder="Describe your legal expertise, practice areas, and how you help clients…" />
              ...
              <div className="p-6 space-y-5">
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-4">
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-400">
                    Provide a link to your bar association practising certificate. Applications without a valid certificate will be rejected.
                  </p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Link to bar certificate *</label>
...
export default function Lawyers() {
  ...
  const userCountry = currentUser?.country || 'GH';
  const regions     = COUNTRY_REGIONS[userCountry] || COUNTRY_REGIONS.GH;
  ...
  useEffect(() => {
    ...
    const params = { country: userCountry };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      {showApply && <ApplyModal onClose={() => setShowApply(false)} userCountry={userCountry} />}
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold text-brand-600 dark:text-brand-400 uppercase tracking-widest">{t('lawyers.verified')}</span>
              </div>
              <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white mb-1">{t('lawyers.title')}</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm leading-relaxed">
                {t('lawyers.subtitle')}
              </p>
            </div>
            ...
            >
              ...
              <span className="hidden sm:inline">{t('lawyers.applyBtn')}</span>
              <span className="sm:hidden">{t('lawyers.applyBtn')}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-5 py-6">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <select value={region} onChange={e => setRegion(e.target.value)} className="input pl-9 text-sm w-full appearance-none">
                <option value="">{t('lawyers.allRegions')}</option>
                {regions.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
            </div>
            <div className="relative flex-1">
              ...
              <select value={specialty} onChange={e => setSpecialty(e.target.value)} className="input pl-9 text-sm w-full appearance-none">
                <option value="">{t('lawyers.allSpecialties')}</option>
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-5 py-14 text-center">
            ...
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{t('lawyers.noResults')}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
              {isFiltered ? t('lawyers.noResultsSub') : 'We are onboarding verified lawyers for your country. Check back soon.'}
            </p>
            {isFiltered && (
              <button onClick={() => { setRegion(''); setSpecialty(''); }} className="btn-primary text-sm">{t('dashboard.clearFilter')}</button>
            )}
          </div>
        ) : (
        ...
        <div className="mt-8 bg-gradient-to-br from-brand-700 to-brand-900 rounded-2xl p-6 text-center relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <p className="font-bold text-white text-base mb-1">Are you a verified lawyer in Africa?</p>
            <p className="text-sm text-white/70 mb-4 max-w-xs mx-auto">
              Join our growing directory. Clients come to you already having reviewed their documents with Klaro.
            </p>
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 text-center mt-5 px-4 leading-relaxed">
          Lawyers are verified members of their respective national bar associations.
          Klaro is not responsible for services provided by listed lawyers.
        </p>
      </main>
    </div>
  );
